...
$(function(){
    ...
    var nowData = {
        ...
        "InstrumentID": id,
        "StartIndex": "0",
        "StartDate": todayDate ? todayDate:"-1",
        "StartTime": todayTime ? todayTime:"0",
        "Count": "0"
    };
    //订阅快照请求
    var HQAll = {
    ...
    var c_data = [];
    var datatime = "1507598320219";
    var yc;

    var kzDate,kzTime;//快照时间
...
//从XML表中摘出时间，name,id，小数位,指数类型
    ...
    // 获取昨收
    // function getYestRecive(){
    //     socket.request(yestRecive);
    // }
    //获取历史指数数据
    ...
    function initCharts(data,type){
        if (data) {
            if(type == "add"){
                if(myChart != undefined){
                    ...
                    var zVale = parseFloat(((parseFloat(a_lastData[0].Last) - parseFloat(yc)) / parseFloat(yc) * 100).toFixed(2)); //行情最新涨跌幅
                    var aValue = parseFloat(a_lastData[0].Volume); //最新成交量

                    for(var i=0;i<c_data.length;i++){
                        if(last_date == c_data[i]){
                            history_data[i] = parseFloat(a_lastData[0].Last);
                            ...
                            if(i > (history_data.length-1) ){
                                for(var j=history_data.length-1;j<=i;j++){
                                    ...
                                    if(j == i){
                                        history_data[j] = parseFloat(a_lastData[0].Last);
                    ...
                    var fvalue, r1;
                    fvalue = parseFloat(a_lastData[0].Last);
                ...
                if(data.d && data.d.length>0){
                            ...
                            if(moment(beginTime).utc().valueOf() < moment(finishTime).utc().valueOf()){
                                ...
                                if(moment(finishTime).utc().valueOf() < moment(beginTime1).utc().valueOf()){
                                    ...
                                    if(moment(beginTime1).utc().valueOf() < moment(finishTime1).utc().valueOf()){
                                        ...
                                        //跨天
                                        finishTime1 = formatDate(data[0].Date) + " " + nowDateTime[1].endTime1;
                                    }
                                }else{
                                    beginTime1 = formatDate(data[0].Date) + " " + nowDateTime[1].startTime1;
                                    finishTime1 = formatDate(data[0].Date) + " " + nowDateTime[1].endTime1;
                                }
                            }else{
                                //结束时间为第二天   跨天了
                                finishTime = formatDate(data[0].Date) + " " + nowDateTime[0].endTime;
                                beginTime1 = formatDate(data[0].Date) + " " + nowDateTime[1].startTime1;
                                finishTime1 = formatDate(data[0].Date) + " " + nowDateTime[1].endTime1;
                            }
                    ...
                    var option = {
                        ...
                        tooltip: {
                            ...
                            formatter: function (params) {
                                ...
                                var tpl = "浦发银行<br />" + formatDate(paramsTime,'1') + "<br />";
                                $.each(params,function(i,items){
                                    switch (items.seriesName){
                                        case "Mline":
                                        tpl += "价格:" + (items.value?items.value:0) + "<br />";
                                        break;
                                        case "limit":
                                        tpl += "涨跌幅:" + (items.value?items.value:0) + "<br />";
                                        break;
                                        case "Volumn":
                                        tpl += "成交量:" + (items.value?items.value:0) + "<br />";
                                        break;
                                        default:
                                        break;
                                    }
                        ...
                        dataZoom: [
                            {
                                ...
                                // startValue:c_data[0],
                                // endValue:c_data[c_data.length-1]
                            },
                            {
                                show: true,
                                xAxisIndex: [0, 1],
                        ...
                        xAxis: [
                            {
                                ...
                                axisLabel: {
                                    ...
                                    formatter: function (value, number) {
                                        var tVal = moment(parseFloat(value)).format("HH:mm");
                                        return tVal;
                                    },
                                    textStyle: {
                                        color: '#999'
                    ...
                    myChart.on('showTip', function (params) {
                        ...
                        if (history_data[mouseHoverPoint]) {
                            ...
                            $("#toolContent_M").children().eq(1).text(history_data[mouseHoverPoint]);
                            $("#toolContent_M").children().eq(2).text(a_history_data[mouseHoverPoint]);
                            $("#toolContent_M").children().eq(3).text(z_history_data[mouseHoverPoint]);
        ...
        function set_marketTool(data) {
            if (!isHoverGraph || isHoverGraph && !history_data[mouseHoverPoint]) {
                ...
                $("#toolContent_M").children().eq(1).text(data[0]);
                $("#toolContent_M").children().eq(2).text(data[2]);
                $("#toolContent_M").children().eq(3).text(data[1]);
            }
        }
    }
